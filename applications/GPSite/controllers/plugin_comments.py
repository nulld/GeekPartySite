...
def index():
		...
		if not comment.deleted:
			return LI(
				DIV(I('"'+comment.body+'"'),BR(),
					T('posted by %s ',comment.created_by.registration_id),
	...
	if parent_id == 'delete':
		...
		itemId = dbco.insert(body=request.vars.body.strip(), deleted=False)
		db.commit()

		item = dbco[itemId]

		print "121212", item.body, item.deleted
		return node(item)
